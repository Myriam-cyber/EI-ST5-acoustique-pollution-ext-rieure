...
def optimization_procedure(domain_omega, spacestep, omega, f, f_dir, f_neu, f_rob,
    ...
    (M, N) = np.shape(domain_omega)
    mask_R = (domain_omega == _env.NODE_ROBIN)
    beta_target = V_obj*N / np.sum(mask_R)
    numb_iter = max_iter
    ...
    for n in range(numb_iter):
        ...
        for j in range(10):
            ...
            chi_tent = project_Uad_star(chi_tent, mask_R, beta_target)
...
def run_optimization_for_level(level, N, f_Hz, V_obj, zeta0, mu1, max_iter, delta):
    ...
    domain_flat ,x , y, _, _ = preprocessing._set_geometry_of_domain(M, N, level=0)
    # Count Robin boundary points
    mask_R = (domain_omega == _env.NODE_ROBIN)
    mask_R_flat  = (domain_flat  == _env.NODE_ROBIN)
...
# =================================================================
    ...
    chi_bin = project_to_binary(chi_relaxed, mask_R, beta_target)
